...
import { auth, issues, dashboard, importCSV } from './services/api';
...
function App() {
  ...
  const loadData = async () => {
    try {
      ...
      const [issuesData, statsData] = await Promise.all([
        issues.getAll(filters),
        dashboard.getStats()
      ]);
      setIssuesList(issuesData.data || []);
  ...
  return (
    <div className="fade-in" style={{ minHeight: '100vh', backgroundColor: '#f3f4f6' }}>
      <div style={{ 
        background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
        color: 'white', 
        padding: '24px',
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <h1 style={{ margin: 0, fontSize: '24px', fontWeight: '700' }}>Trial Issue Tracker</h1>
              <p style={{ margin: '2px 0 0 0', fontSize: '13px', opacity: 0.9 }}>Clinical Trial Management</p>
            </div>
          </div>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 20px' }}>
        ...
        {stats && (
          <div className="slide-in" style={{ marginBottom: '32px' }}>
            <h2 style={{ marginBottom: '20px', fontSize: '20px', fontWeight: '700', color: '#1f2937', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span>📊</span> Dashboard Overview
            </h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: '16px' }}>
              <StatCard title="Open" count={stats.byStatus.open} color="#ef4444" icon="🔴" />
              <StatCard title="In Progress" count={stats.byStatus.in_progress} color="#f59e0b" icon="⏳" />
              <StatCard title="Resolved" count={stats.byStatus.resolved} color="#10b981" icon="✅" />
              <StatCard title="Minor" count={stats.bySeverity.minor} color="#6b7280" icon="ℹ️" />
              <StatCard title="Major" count={stats.bySeverity.major} color="#f59e0b" icon="⚠️" />
              <StatCard title="Critical" count={stats.bySeverity.critical} color="#ef4444" icon="🚨" />
            </div>
          </div>
        )}

        <div style={{ display: 'flex', gap: '12px', marginBottom: '24px', flexWrap: 'wrap' }}>
          <button onClick={() => { setShowCreateForm(true); setEditingIssue(null); setIssueForm({ title: '', description: '', site: '', severity: 'minor', status: 'open' }); }} style={buttonStyle('#667eea')}>
            ➕ Create Issue
          </button>
          ...
          <button onClick={loadData} style={buttonStyle('#6b7280')} disabled={loading}>
...
function StatCard({ title, count, color, icon }) {
  return (
    <div className="stat-card" style={{ 
      ...
      <div style={{ position: 'absolute', top: '16px', right: '16px', fontSize: '32px', opacity: 0.15 }}>
        {icon}
      </div>
      <div style={{ fontSize: '12px', color: '#9ca3af', marginBottom: '8px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.8px' }}>
        {title}
      </div>
      <div style={{ fontSize: '40px', fontWeight: '800', color, lineHeight: 1 }}>
        {count}
      </div>
    </div>
  );
